...
def handle_leave_submit(doc, method=None):
    """
    Trigger when Leave Application is submitted (Pending state).
    Sends mail to HR with a modern, professional design.
    """
    ...
    leave_details = f"""
        <table style="width:100%; border-collapse:collapse; font-size:14px;">
            ...
            {permission_row}
            <tr><td style="padding:8px 10px; font-weight:bold; background:#f9f9f9;">Reason</td><td style="padding:8px 10px;">{getattr(doc, 'reson', '') or ''}</td></tr>
        </table>
    """

    # Modern styled HTML email
    message = f"""
    <div style="font-family: 'Montserrat', 'Segoe UI', Arial, sans-serif; background:#f4f6f8; padding:40px;">
        <div style="max-width:600px; margin:auto; background:white; border-radius:12px; 
                    box-shadow:0 2px 8px rgba(0,0,0,0.08); overflow:hidden;">
            
            <div style="background:#007bff; color:white; padding:18px 24px; font-size:18px; 
            ...
            <div style="padding:24px; color:#333;">
                ...
                <p style="font-size:14px; line-height:1.6;">
                    A new leave application has been submitted by <b>{doc.employee_name}</b> and is 
    ...
    try:
        frappe.sendmail(
            recipients=[hr_email],
            subject=f"Leave Approval Request for {doc.employee_name}",
            message=message,
            reference_doctype="Leave Application",
            reference_name=doc.name
        )
    except Exception as e:
        frappe.log_error(f"Leave Submit Mail Error: {str(e)}", "Leave Email Debug")

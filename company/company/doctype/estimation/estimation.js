frappe.ui.form.on("Estimation",
    {
        refresh(frm) {
            ...
            if (frm.doc.client_name) {
                frappe.db.get_value("Contacts", frm.doc.client_name, ["company_name", "first_name", "address"], (r) => {
                    if (r) {
            ...
            add_edit_customer_button(frm, "estimation_details_section");

        },

        client_name(frm) {
            // Update button visibility/link when client changes
            add_edit_customer_button(frm, "estimation_details_section");
        },
        ...
        validate(frm) {
            ...
            if (!frm.doc.table_qecz || frm.doc.table_qecz.length === 0) {
                frappe.msgprint({
                    ...
                    message: __('Please add at least one item to the Estimation'),
                    indicator: 'red'
                });
